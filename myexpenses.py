...
def init():
    connectionObjn = db.connect("expenseTrackerss.db")
...
def viewexpense():
    ...
    print(rows)
    print(amount)
    l = Label(root, text="Date\t\t  Name\t\t  Title\t\t  Expense", font=('arial', 15, 'bold'), bg="Orange", fg="white")
    l.grid(row=4, column=30, padx=7, pady=7)
    ...
    st += "\n"
    if rows:
        st += solve("TOTAL-") +  " "*80+ solve(str(amount[0]))
    print(st)
    l = Label(root, text=st, font=('arial', 12))
    l.grid(row=5, column=30, padx=7, pady=7)

def solve(n):
    x = len(n)
    mid = (40-x)
    return " "*(mid//2) + n + " "*(mid - (mid//2))
